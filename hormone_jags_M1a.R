...
modelName <- "Model_Hormone_1a.txt"
...
datSDB$SCL1 <- (datSDB$SCL - mean(datSDB$SCL))/sd(datSDB$SCL)
datBDAPAN$SCL1 <- datBDAPAN$SCL - mean(datBDAPAN$SCL)/sd(datSDB$SCL)
...
initsFunction <- function(n){
  ...
  beta_DOY <- rnorm(1, 0, 0.01)
  beta_SCL <- rnorm(1, 0, 0.01)
...
g.diag <- gelman.diag(zm)  # get an error because of correlations
...
mu1 <- unlist(zm[,varnames(zm) == 'mu1'])
mu2 <- unlist(zm[,varnames(zm) == 'mu2'])
...
p1 <- vector(length = dim(datSDB)[1], mode = 'numeric')
for (i in 1:dim(datSDB)[1]){
  t <- exp(mean(beta0) + 
    mean(betaH) * mean(c(datSDB$Testo1_mean0[i] + datSDB$Testo2_mean0[i])) +
    mean(betaDOY) * datSDB$DOY[i] + 
    mean(betaSCL) * datSDB$SCL1[i])
  
  p1[i] <- t/(1+t)
}

p2 <- vector(length = dim(datBDAPAN)[1], mode = 'numeric')
for (i in 1:dim(datBDAPAN)[1]){
  t <- exp(mean(beta0) + 
    mean(betaH) * mean(c(datBDAPAN$Testo1_mean0[i], datBDAPAN$Testo2_mean0[i])) + 
    mean(betaDOY) * datBDAPAN$DOY[i] + 
    mean(betaSCL) * datBDAPAN$SCL1[i])
