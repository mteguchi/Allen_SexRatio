...
# including Sex1 is not good for looking at convergence.
# so this needs to be re-run with Sex1 and Sex2 that goes into
# different output file. 
if (save == TRUE) {
  if (length(grep("Sex1", parameters)) > 0){
    saveFname <- paste(D$Rdir, 
                       "Allen_SexDetermination/RData/Hormone_", 
                       runDate, "M3_SclDoyWithSex.RData", sep = "")    
  } else {
    saveFname <- paste(D$Rdir, 
                       "Allen_SexDetermination/RData/Hormone_", 
                       runDate, "M3_SclDoyWithoutSex.RData", sep = "")    
  }
}
# define model and data file names
modelName <- "Model_Hormone_3_SclDOY.txt"
...
H1 <- array(data = NA, dim = c(length(uniqIDs), 
                               max(datSDB$Rep), 2))
Tail1 <- DOY1 <- SCL1 <- matrix(data=NA, 
                                nrow = length(uniqIDs), 
                                ncol = max(datSDB$Rep))
...
initsFunction <- function(ns){
  ...
  beta_DOY1 <- rnorm(1, 0, 10)  
  beta_DOY2 <- rnorm(1, 0, 10)
  beta_SCL <- rnorm(1, 0, 10)
...
## uncomment the diag statements when checking for convergence
## without Sex1 and Sex2 in parameter argument. The binary
## outcome is not useful in computing diagnostic statistics
