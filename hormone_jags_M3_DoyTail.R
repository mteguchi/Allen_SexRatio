...
if (save == TRUE) {
  if (length(grep("Sex1", parameters)) > 0){
    saveFname <- paste(D$Rdir, 
                       ...
                       runDate, "M3_DoyTailWithSex.RData", sep = "")
  } else {
    saveFname <- paste(D$Rdir, 
                       "Allen_SexDetermination/RData/Hormone_", 
                       runDate, "M3_DoyTailWithoutSex.RData", sep = "") 
  }
}  
# define model and data file names
modelName <- "Model_Hormone_3_DoyTail.txt"
...
#datSDB$Tail_mean0 <- datSDB$Tail - mean(datSDB$Tail, na.rm=T)
